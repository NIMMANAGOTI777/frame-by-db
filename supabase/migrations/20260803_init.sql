...
-- 6. INITIALIZE STORAGE BUCKETS & DEFINE STORAGE POLICIES
...
DROP POLICY IF EXISTS "Allow public select on videos" ON storage.objects;
DROP POLICY IF EXISTS "Admin full storage access" ON storage.objects;
DROP POLICY IF EXISTS "Client read own albums bucket" ON storage.objects;
...
VALUES 
  ...
  ('albums', 'albums', false) -- Private albums for clients
ON CONFLICT (id) DO UPDATE SET public = EXCLUDED.public;

-- Public Storage Read Policies (Images, Videos)
CREATE POLICY "Allow public select on images" ON storage.objects FOR SELECT TO anon, public USING (bucket_id = 'images');
CREATE POLICY "Allow public select on videos" ON storage.objects FOR SELECT TO anon, public USING (bucket_id = 'videos');

-- Admin Storage Policies (Upload, Update, Delete in all buckets)
CREATE POLICY "Admin full storage access" ON storage.objects FOR ALL TO authenticated USING (is_admin());

-- Client Album Storage Read Policy (Restricts clients to read only their own albums path: e.g. albums/clientId/...)
CREATE POLICY "Client read own albums bucket" ON storage.objects FOR SELECT TO authenticated 
USING (
  bucket_id = 'albums' 
  AND (
    EXISTS (
